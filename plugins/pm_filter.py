...
from info import ADMINS, AUTH_CHANNEL, AUTH_USERS, SUPPORT_CHAT_ID, CUSTOM_FILE_CAPTION, MSG_ALRT, PICS, AUTH_GROUPS, P_TTI_SHOW_OFF, GRP_LNK, CHNL_LNK, NOR_IMG, LOG_CHANNEL, SPELL_IMG, MAX_B_TN, IMDB, \
    SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, NO_RESULTS_MSG, IS_VERIFY, HOW_TO_VERIFY, NO_RESULT
from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton, CallbackQuery, InputMediaPhoto
from pyrogram import Client, filters, enums
from pyrogram.errors import FloodWait, UserIsBlocked, MessageNotModified, PeerIdInvalid
from utils import get_size, is_subscribed, get_poster, search_gagala, temp, get_settings, save_group_settings, get_shortlink, send_all, check_verification, get_token, replace_username
...
async def next_page(bot, query):
    ...
    if ENABLE_SHORTLINK and settings['button']:
        btn = [
            [
                InlineKeyboardButton(
                    text=f"[{get_size(file.file_size)}] {replace_username(file.file_name)}", url=await get_shortlink(query.message.chat.id, f"https://telegram.me/{temp.U_NAME}?start=pmshort_{key}")
                ),
            ]
            for file in files
        ]
        ...
        btn = [
            [
                InlineKeyboardButton(
                    text=f"{replace_username(file.file_name)}", url=await get_shortlink(query.message.chat.id, f"https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}")
                ),
                InlineKeyboardButton(
    ...
    if files:
        ...
        if ENABLE_SHORTLINK and settings['button']:
            btn = [
                [
                    InlineKeyboardButton(
                        text=f"[{get_size(file.file_size)}] {replace_username(file.file_name)}", url=await get_shortlink(query.message.chat.id, f"https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}")
                    ),
                ]
                for file in files
            ]
...
async def advantage_spoll_choker(bot, query):
    ...
    if k == False:
        ...
        if files:
            ...
            if NO_RESULTS_MSG:
                await bot.send_message(chat_id=NO_RESULT, text=(script.NORSLTS.format(reqstr.id, reqstr.mention, movie)))
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
               ...
               #         )
                    ...
                    return await query.answer(f"Hᴇʏ {query.from_user.first_name}, Tʜɪs Is Nᴏᴛ Yᴏᴜʀ Mᴏᴠɪᴇ Rᴇǫᴜᴇsᴛ. Rᴇǫᴜᴇsᴛ Yᴏᴜʀ's !", show_alert=False)
...
async def auto_filter(client, msg, spoll=False):
        ...
        if len(message.text) < 100:
            ...
            if not files:
                if settings["spell_check"]:
                    ...
                    if NO_RESULTS_MSG:
                        await client.send_message(chat_id=NO_RESULT, text=(script.NORSLTS.format(reqstr.id, reqstr.mention, search)))
                    return
        else:
            return
    else:
    ...
    searching = await message.reply_text(f"<i>sᴇᴀʀᴄʜɪɴɢ ғᴏʀ {message.text}</i>")
    settings = await get_settings(message.chat.id)
    ...
    if ENABLE_SHORTLINK and settings["button"]:
        btn = [
            [
                InlineKeyboardButton(
                    text=f"[{get_size(file.file_size)}] {replace_username(file.file_name)}", url=await get_shortlink(message.chat.id, f"https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}")
                ),
            ]
            for file in files
        ]
    ...
    if imdb:
        ...
        cap = f"<b>📟 ᴍᴏᴠɪᴇ ɴᴀᴍᴇ: <code>{search}</code>\n🙋‍♂️ ʀᴇqᴜᴇsᴛᴇᴅ ʙʏ: {message.from_user.mention}\n\nɪꜰ ᴀɴʏ ᴍᴏᴠɪᴇ ɪs ɴᴏᴛ ᴀᴠᴀɪʟᴀʙʟᴇ - ᴀsᴋ ɪɴ ᴛʜɪs ꜰᴏʀᴍᴀᴛ\n\nᴠɪᴋʀᴀᴍ 2022\nᴠɪᴋʀᴀᴍ 2022 1080\n\nᴀsᴋ ʜᴇʀᴇ - @KPT_Help_bot</b>"
    if imdb and imdb.get('poster'):
        try:
            ...
            logger.exception(e)
            fek = await message.reply(caption=cap, reply_markup=InlineKeyboardMarkup(btn))
            try:
                if settings['auto_delete']:
                    await asyncio.sleep(600)
                    await fek.delete()
        ...
        fuk = await searching.edit(text=cap, reply_markup=InlineKeyboardMarkup(btn))
        try:
            if settings['auto_delete']:
                await asyncio.sleep(600)
                await fuk.delete()
...
async def advantage_spell_chok(client, msg):
    ...
    try:
        ...
        if NO_RESULTS_MSG:
            await client.send_message(chat_id=NO_RESULT, text=(script.NORSLTS.format(reqstr.id, reqstr.mention, mv_rqst)))
